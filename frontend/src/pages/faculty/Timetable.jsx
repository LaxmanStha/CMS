...
export default function FacultyTimetable() {
  ...
  const teachersMap = useMemo(() => {
    ...
    timetable.forEach(t => {
      if (!map.has(t.teacherId)) {
        map.set(t.teacherId, { name: t.teacherName, department: t.department, assignments: [] });
      }
      map.get(t.teacherId).assignments.push(t);
    });
    return map;
  }, [timetable]);
  ...
  return (
    <div className="space-y-6">
      ...
      {Array.from(teachersMap.entries()).map(([teacherId, teacher]) => (
